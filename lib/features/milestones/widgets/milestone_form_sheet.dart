...
class _MilestoneFormSheetState extends ConsumerState<_MilestoneFormSheet> {
  ...
  void initState() {
    ...
    _bonusController = TextEditingController(
      text: (m?.completionPoints ?? 0).toString(),
    );
    ...
    CosmeticsService.unlockedPaletteIds().then((ids) {
      if (mounted) {
        setState(
          () => _paletteLength = AppColors.milestonePalette.length + ids.length,
        );
      }
    });
  }
  ...
  Future<void> _pickDate() async {
    ...
    var initial = _targetDate ?? today.add(const Duration(days: 30));
    var first = today;
    if (initial.isBefore(first)) {
      first = DateTime(initial.year, initial.month, initial.day);
    }
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty) {
      setState(() => _error = 'Give your milestone a name');
      return;
    }
    setState(() {
      _saving = true;
    ...
    try {
      ...
      debugPrint('Milestone save failed: $e\n$st');
      if (mounted) {
        setState(() {
          _saving = false;
          _error = 'Could not save this milestone. Try again.';
        });
      }
    }
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              padding: EdgeInsets.fromLTRB(24, 8, 24, 16 + safeBottom),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  if (_error != null) ...[
